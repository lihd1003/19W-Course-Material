...
int read_from(int client_index, struct sockname *usernames) {
    ...
    buf[num_read] = '\0';
    char send[BUF_SIZE * 2 + 2];
    strcpy(send, usernames[client_index].username);
    strcat(send, ": ");
    strcat(send, buf);
    strncpy(buf, send, BUF_SIZE);
    buf[BUF_SIZE] = '\0';
